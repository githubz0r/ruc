---
title: "10 - Multifactorial design"
author: "John Shorter"
date: "Mar/05/2026"
output:
  html_document:
...
```{r}
Potato_Yield <- read.csv("https://raw.githubusercontent.com/johnshorter/TeachingData/main/Potato_yield.csv")
head(Potato_Yield)
str(Potato_Yield)
...
Potato_Yield %>% 
  ...
  stat_summary(geom = "line",  
               fun = mean,  
               aes(group = Variety, color = Variety),  
               size = 1.2) + 
  stat_summary(geom = "ribbon",   
               fun.data = mean_se, 
               aes(group = Variety,
                   fill = Variety), 
               alpha = 0.5) +   
  geom_point(aes(fill = Variety), 
             position = position_jitter(0.3, seed = 666),    
             alpha = 0.8,             
             size = 3,
             shape = 21, 
             color = "black") +    
  scale_fill_manual(values = wisteria[c(1, 7)]) +  
  scale_color_manual(values = wisteria[c(1, 7)]) + 
  scale_x_continuous(breaks = c(0, 90, 150, 210, 270)) +  
  labs(x = "Nitrogen",              
       y = "Yield",
       fill = "Variety",
       color = "Variety") +
...
```{r}
potato <- Potato_Yield %>% mutate(Nitrogen = as.factor(Nitrogen))
potato %>% group_by(Nitrogen, Variety) %>% summarize(mean=mean(Yield), var=var(Yield), n = n())
```


```{r}
model_yield <- lm(Yield ~ Nitrogen + Variety + Nitrogen:Variety, data = potato) 
...
Residuals vs fitted looks quite good, showing linearity among the values. QQ plot of residuals also looks good. Scale location has a bit of a slope so we're not quite homoskedastic. Higher values of Nitrogen tend to lead to higher variance. It's not extreme though.
...
anova(model_yield)
```
Both factors, including their interaction have significant F values, i.e. the different factors, including their combinations/interactions, have different means, but Variety has the highest F value and thus explains most of the variance in the data.
##  Tukey test
...
```{r}
estimate_per_nitrogen <- emmeans(model_yield, pairwise ~ Variety|Nitrogen)
estimate_per_nitrogen$contrast
```

Pull out the cld output and interpret the results.

```{r}
cld(estimate_per_nitrogen, Letters=letters)
```

It shows that at each level of Nitrogen, the different Varieties have different means.
...
**File name format:** `Lastname_Firstname_L10.html`
...
### Exercise 2 — Two-way model (main effects)
Fit a model with two predictors (factors) and report main effects.

**Self-check:** model includes both factors; you interpret at least one coefficient or ANOVA term.

### Exercise 3 — Interaction
Fit an interaction model and decide whether interaction is meaningful.
Make an interaction plot.

**Self-check:** you state whether interaction is present and back it with output + plot.

### Exercise 4 — Simple effects / follow-up
If interaction exists (or even if weak), do one follow-up comparison (e.g., compare groups within one level).

**Self-check:** you explain what comparison you ran and why.

### Exercise 5 — Report-ready summary
Write a short “Results” paragraph (5–7 lines) that mentions main effects and interaction (if present).

**Self-check:** paragraph is readable and avoids overclaiming.
