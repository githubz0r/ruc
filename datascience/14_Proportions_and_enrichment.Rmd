---
title: "14 - Proportions and enrichment"
author: "John Shorter"
date: "Mar/19/2026"
output:
  html_document:
...
```{r}
# Perform fisher.test
fisher_result <- fisher.test(contingency_table) # just extend the contingency table for more values
...
ggplot(diamonds, aes(x = cut, fill = color)) +
  ...
  labs(
    title = "Frequency of Cut by Color",
    x = "Cut",
    y = "Count",
    fill = "Color"
  ) +
...
# Perform chi-square test
chi_res_diamonds <- chisq.test(diamonds_table)
...
**File name format:** `Lastname_Firstname_L14.html`
...
### Exercise 2 — Two proportions test
Compare two proportions (chi-square or Fisher’s exact, depending on counts).
Interpret the result.

**Self-check:** you show a 2x2 table and the test output.

### Exercise 3 — Odds ratio (manual check)


Compute an odds ratio from a 2x2 table (either using a function or by hand).
Confirm it matches (approximately) what the test reports.

```{r}
fisher.test(x = rbind(c(33, 14),
                      c(23,5))) 
...
**Self-check:** manual OR and reported OR are close; you show your calculation.

### Exercise 4 — Enrichment idea
Run (or simulate) a simple enrichment-style test (hypergeometric framing is fine).
Explain what “enriched” means in your context. (Typical use case gene set enrichment)

#### 

if necessary I can just do some DNA encoded library example where e.g. certain moieties are enriched after the experiment (or use gemini to generate the values for us). Note that sequences represent DNA barcodes attached to chemical building blocks.
Prompt: simulate a small DNA encoded library experiment. Row 1: count of a few different sequences. Row2: count of sequences after the assay
then calculate chi-square and fishers exact test
```{r}
# Create the contingency table
# Each column is a sequence, Each row is a stage
del_matrix <- matrix(c(250, 40,   # Seq_A
                       260, 35,   # Seq_B
                       240, 100,  # Seq_C
                       250, 45),  # Seq_D
                     nrow = 2, 
                     dimnames = list(Stage = c("Input", "Assay"),
                                     Sequence = c("Seq_A", "Seq_B", "Seq_C", "Seq_D")))

# 1. Chi-Square Test
chi_result <- chisq.test(del_matrix)

# 2. Fisher's Exact Test 
# (Note: For very large tables, simulate.p.value = TRUE may be needed)
#fisher_result <- fisher.test(del_matrix, simulate.p.value = T)
...
**Self-check:** you define background vs selected set clearly.

### Exercise 5 — Visualization
Make a clear bar plot (with uncertainty if possible) for proportions and annotate with group sizes.

```{r}
library(scales)

# compute proportions, SE, and 95% CI by cut and color
df_prop <- diamonds %>%
  count(cut, color) %>%
  group_by(cut) %>%
  mutate(
...
# compute total N and max upper per cut for annotation
df_N <- df_prop %>%
  group_by(cut) %>%
  summarise(
    N_total = first(N),
...
# plot
ggplot(df_prop, aes(x = cut, y = prop, fill = color)) +
  geom_col(position = position_dodge(width = 0.8)) +
  geom_errorbar(aes(ymin = lower, ymax = upper),
                position = position_dodge(width = 0.8),
                width = 0.2) +
  geom_text(data = df_N,
            aes(x = cut, y = ymax + 0.02, label = paste0("N=", N_total)),
            inherit.aes = FALSE) +
  scale_y_continuous(labels = percent_format(), expand = expansion(mult = c(0, 0.1))) +
  labs(
    title = "Proportion of Diamonds by Cut and Color with 95% CI",
    x = "Cut",
    y = "Proportion",
    fill = "Color"
  ) +
...
**Self-check:** plot shows proportions (0–1 or 0–100%) and includes n.
