...
**File name format:** `Lastname_Firstname_L07.html`
...
### Exercise 2 — A different outcome
Repeat the one-way ANOVA workflow using a different numeric outcome (same dataset).

**Self-check:** you show a plot + model output + ANOVA table.

### Exercise 3 — Assumptions and alternatives
Check assumptions (normality of residuals and equal variance).
If one assumption looks poor, try one reasonable alternative (transformation or non-parametric test) and compare conclusions.

**Self-check:** you show at least one diagnostic plot and explain what changed.

### Exercise 4 — Report like a paper (short)
Write 3–5 sentences reporting:
- what groups were compared
- the test used
- the main result (direction + p-value)
- what post-hoc test showed

**Self-check:** your report includes both statistics and interpretation.

### Exercise 5 — Mini-transfer
Pick a different built-in dataset (e.g., `iris`) and run one-way ANOVA on a sensible outcome by group.

**Self-check:** group variable is a factor; you include a plot and an ANOVA table.
