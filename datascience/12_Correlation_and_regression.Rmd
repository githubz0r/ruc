---
title: "12 - Correlation and regression"
author: "John Shorter"
date: "Mar/12/2026"
output:
  html_document:
...
```{r}
child_mortality <- read_csv("https://raw.githubusercontent.com/johnshorter/TeachingData/main/child_mortality_0_5_year_olds_dying_per_1000_born.csv", col_types = cols()) 
babies_per_woman <- read_csv("https://raw.githubusercontent.com/johnshorter/TeachingData/main/children_per_woman_total_fertility.csv", col_types = cols()) 
income <- read_csv("https://raw.githubusercontent.com/johnshorter/TeachingData/main/income_per_person_gdppercapita_ppp_inflation_adjusted.csv", col_types = cols())  
```
...
**File name format:** `Lastname_Firstname_L12.html`
...
### Exercise 2 — Correlation (and plot)
Compute a correlation between two numeric variables and make a scatter plot.
Add a sentence interpreting sign and strength.

**Self-check:** correlation is a number between -1 and 1; plot matches variables.

### Exercise 3 — Fit a linear model and interpret slope
Fit a linear regression and interpret the slope in words.

**Self-check:** you state “per 1 unit increase in X, Y changes by … (on average)”.

### Exercise 4 — Diagnostics
Check at least two diagnostics (residual plot, QQ plot, leverage).
State whether anything looks concerning.

**Self-check:** you include the plots and a short verdict.

### Exercise 5 — Prediction with uncertainty
Make a prediction for a new value of X and include an uncertainty interval.

**Self-check:** you report both predicted value and an interval.
