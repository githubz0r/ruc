...
What are the types of comparisons where we would want to use logistic regression?
...
**File name format:** `Lastname_Firstname_L13.html`
...
### Exercise 2 — Fit and interpret odds ratios
Fit a logistic regression and report at least one odds ratio (OR).
Interpret it in plain language.

**Self-check:** OR is positive; interpretation matches direction (OR>1 vs OR<1).

### Exercise 3 — Predicted probabilities
Generate predicted probabilities for at least 5 observations (or a grid of predictor values).
Plot probability vs predictor.

**Self-check:** probabilities are between 0 and 1; plot shows a sensible trend.

### Exercise 4 — Confusion matrix at a threshold
Choose a threshold (e.g., 0.5) and create a confusion matrix.
Compute accuracy and either sensitivity or specificity.

**Self-check:** confusion matrix counts add up to N; metrics are between 0 and 1.

### Exercise 5 — Threshold trade-off (qualitative)
Try a second threshold (e.g., 0.3 or 0.7) and explain what changed and why.

**Self-check:** you correctly describe trade-offs (false positives vs false negatives).
